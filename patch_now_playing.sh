...
+        boolean changed = (mSpectralPanel != spectralPanel || mBroadcastStatusPanel != broadcastStatusPanel || mResourceStatusPanel != resourceStatusPanel);
+
...
-        setupWidgets();
+        if (changed || mWidgetContainer.getComponentCount() == 0) {
+            setupWidgets();
+        }
     }
